...
// Tabelas de preço por faixa (maxInclusive, tarifa/unidade)
// Faixa 1 termina em 999; Faixa 2 começa em 1000 (= prevMax + 1)
const TABELAS = {
  ligacao: [
    [999, 0.25],
    [4_999, 0.22],
...
// =======================
// Núcleo de faixas (min/max)
// =======================

/**
 * Dado um custo total (R$), encontra a quantidade no DESTINO
 * respeitando MIN e MAX de cada faixa e escolhendo a ÚLTIMA faixa válida.
 */
function resolverQuantidadePorCusto(tipo, custo) {
  ...
  for (let i = 0; i < faixas.length; i++) {
    ...
    const min = i === 0 ? 1 : faixas[i - 1][0] + 1; // início da faixa (1, 1000, 5000, ...)

    const q = custo / tarifa; // não arredondar aqui; exibimos arredondado depois
    if (q >= min && q <= max) {
      escolhido = { quantidade: q, tarifa, min, max };
      // sem "break": queremos a ÚLTIMA faixa possível (tarifa menor)
    }
  }
...
/**
 * Modo "Por dinheiro": converte orçamento em créditos,
 * respeitando MIN/MAX e escolhendo a ÚLTIMA faixa válida.
 */
function calcularCreditosPorDinheiro(tipo, valorBRL) {
  ...
  for (let i = 0; i < faixas.length; i++) {
    ...
    if (qtd > 0 && qtd >= min && qtd <= max) {
      ...
      // segue procurando para pegar a ÚLTIMA faixa válida
    }
  }
...
function calcularPorCreditos() {
  ...
  // origem = destino → identidade (ex.: 1000 ligacao → 1000 ligacao)
  ...
  // destino diferente → resolve pela última faixa válida
...
// Interface única do botão CALCULAR
function calcular() {
...
function setupSeletores() {
  ...
  tabs.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      // limpa resultado ao trocar de modo
      renderResultado("");
    });
  });

  // Máscara leve para BRL (aceita números e vírgula)
  const vr = document.getElementById("valor-reais");
  if (vr) {
    vr.addEventListener("input", () => {
      // Permite apenas dígitos, ponto e vírgula; mantém o que digitar
...
// Expor no escopo global (botões do HTML)
